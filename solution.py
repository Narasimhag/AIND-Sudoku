...
#variables for cols and rows
rows = 'ABCDEFGHI'
cols = '123456789'
cols1 = cols[::-1] #variable to hold reverse order of cols to build diagonals

def cross(A, B):
    "Cross product of elements in A and elements in B."
    return [s+t for s in A for t in B]

#calculating boxes, units and peers
boxes = cross(rows, cols)

row_units = [cross(r, cols) for r in rows]
column_units = [cross(rows, c) for c in cols]
square_units = [cross(rs, cs) for rs in ('ABC','DEF','GHI') for cs in ('123','456','789')]
#diagonal_units is a list which contains both the diagonal boxes
diagonal_units = [[rows[i]+cols[i] for i in range(len(rows))]] + [[rows[i]+cols1[i] for i in range(len(rows))]]
